from ursina import Entity, Vec3, color, held_keys, mouse, camera, scene, destroy, load_model, invoke
...
class GEntity(Entity):
    def __init__(self, *, debug=False, **kwargs):
        ...
        # Apply filtering after model loads
        if self.model:
            self._set_filtering()
        else:
            invoke(self._set_filtering, delay=0.1)  # delay a bit for async model load
